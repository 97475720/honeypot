...
class Swift_MessageTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testBodySwap()
    {
        ...
        $html->getHeaders()->addTextHeader('X-Article-Remove', 'Article-Value');
        $html->getHeaders()->addTextHeader('X-Article-Alter', 'Article-Value');
        ...
        foreach ($message2->getChildren() as $child) {
            $child->setBody('Test');
            $child->getHeaders()->removeAll('X-Article-Remove');
            $child->getHeaders()->get('X-Article-Alter')->setValue('Altered');
        }
