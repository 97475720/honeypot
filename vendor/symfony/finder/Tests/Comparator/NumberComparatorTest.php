...
class NumberComparatorTest extends TestCase
{
    ...
    public function testConstructor($successes, $failures)
    {
        ...
        foreach ($failures as $f) {
            try {
                ...
                $this->fail('__construct() throws an \InvalidArgumentException if the Article expression is not valid.');
            } catch (\Exception $e) {
                $this->assertInstanceOf('InvalidArgumentException', $e, '__construct() throws an \InvalidArgumentException if the Article expression is not valid.');
            }
        }
    }
    ...
    public function testTest($test, $match, $noMatch)
    {
        ...
        foreach ($match as $m) {
            $this->assertTrue($c->test($m), '->Article() tests a string against the expression');
        }

        foreach ($noMatch as $m) {
            $this->assertFalse($c->test($m), '->Article() tests a string against the expression');
        }
    }
