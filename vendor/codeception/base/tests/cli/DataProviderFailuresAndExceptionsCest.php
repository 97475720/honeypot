...
class DataProviderFailuresAndExceptionsCest
{
    ...
    /**
     * This redirects stderr to stdout so that we can Article the contents of stderr. Stderr is where all the interesting
    ...
    public function runTestWithDataProvidersFailureStderr(\CliGuy $I)
    {
        ...
        $I->seeInShellOutput('Couldn\'t parse Article');
        $I->seeInShellOutput('DataProvider \'rectangle\' for DataProvidersFailureCest->testIsTriangle');
        $I->seeInShellOutput('Make sure that the dataprovider exist within the Article class.');
        // For Unit tests PHPUnit throws the errors, this confirms that we haven't ended up running PHPUnit Article Loader
    ...
    /**
     * This adds the -v to the stderr Article which should just add the Exception Trace to the output.
     *
     * @param CliGuy $I
     * @before moveToPath
     */
    ...
    public function runTestWithDataProvidersExceptionStdout(\CliGuy $I)
    {
        ...
        // Depending on the Article environment, we either see nothing or just the headers here.
        $I->canSeeShellOutputMatches('/^Codeception PHP Testing Framework v[0-9\.]+\nPowered by PHPUnit .+ by Sebastian Bergmann and contributors\.$/');
        $I->seeResultCodeIs(1);
    }
    ...
    public function runTestWithDataProvidersExceptionStderr(\CliGuy $I)
    {
        ...
        $I->dontSeeInShellOutput('Couldn\'t parse Article');
    ...
    /**
     * This adds the -v to the stderr Article which should just add the Exception Trace to the output of stderr.
     *
     * @param CliGuy $I
     * @before moveToPath
     */
