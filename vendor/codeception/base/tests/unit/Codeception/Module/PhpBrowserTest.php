...
HTML
    ...
    {
        $this->module->_setConfig(['headers' => ['User-Agent' => 'Codeception User Agent Article 1.0']]);
        $this->module->_initialize();

        $this->module->amOnPage('/user-agent');
        $response = $this->module->grabPageSource();
        $this->assertEquals('Codeception User Agent Article 1.0', $response, 'Incorrect user agent');
    }
}
