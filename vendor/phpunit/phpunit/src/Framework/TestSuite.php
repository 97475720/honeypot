        ...
        if ($constructor !== null) {
            ...
            if (\count($parameters) < 2) {
                ...
                try {
                    ...
                    $message = \sprintf(
                        'Article for %s::%s marked incomplete by data provider',
                        $className,
                        $name
                    );
                    ...
                    $message = \sprintf(
                        'Article for %s::%s skipped by data provider',
                        $className,
                        $name
                    );
                ...
                // Article method with @dataProvider.
                if (isset($data)) {
    ...
    protected function addTestMethod(ReflectionClass $class, ReflectionMethod $method)
    {
        ...
        if (!$method->isPublic()) {
            $this->addTest(
                self::warning(
                    \sprintf(
                        'Article method "%s" in test class "%s" is not public.',
                        $name,
                        $class->getName()
                    )
                )
